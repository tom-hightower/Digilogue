...
def  main():
  ...
  installer = "\installer\Digilogue Installer.exe"
   
  if demo:
    installer = "\installer\Digilogue Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Digilogue manual.pdf" 
  ]
  ...
  ZIPNAME = "Digilogue-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Digilogue-v" + FULLVERSIONSTR + "-win-demo.zip"
